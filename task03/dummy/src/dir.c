 ...
 */
static int __dcache_readdir(struct file* file,
  ...
  if (ctx->pos > 2) {
    ...
    while (count > 0) {
      u64 step = count >> 1;
      dentry = __dcache_find_get_entry(parent, idx + step, &cache_ctl);
      if (!dentry) {
        /* use linear search */
        idx = 0;
        break;
  ...
  for (;;) {
    ...
    spin_lock(&dentry->d_lock);
    di = ceph_dentry(dentry);
    if (d_unhashed(dentry) || d_really_is_negative(dentry) ||
...
out:
  ...
  if (last) {
    int ret;
    di = ceph_dentry(last);
    ret = note_last_dentry(fsc, dfi, last->d_name.name, last->d_name.len,
...
static int ceph_readdir(struct file* file, struct dir_context* ctx) {
  ...
  if (ctx->pos == 0) {
    ...
    if (!dir_emit(ctx, ".", 1, ceph_present_inode(inode), inode->i_mode >> 12))
      return 0;
    ctx->pos = 1;
  }
  if (ctx->pos == 1) {
  ...
  if (ceph_test_mount_opt(fsc, DCACHE) &&
      ...
      ceph_snap(inode) != CEPH_SNAPDIR && __ceph_dir_is_complete_ordered(ci) &&
...
more:
  ...
  if (need_send_readdir(dfi, ctx->pos)) {
    ...
    if (is_hash_order(ctx->pos)) {
      ...
      if (frag == (unsigned)-1)
        frag = ceph_choose_frag(ci, fpos_hash(ctx->pos), NULL, NULL);
    } else {
    ...
    if (dfi->last_name) {
      ...
      req->r_args.readdir.offset_hash = cpu_to_le32(fpos_hash(ctx->pos));
    }
    ...
    doutc(cl,
          ...
          inode, ceph_vinop(inode), err, frag, (int)req->r_reply_info.dir_end,
    ...
    if (test_bit(CEPH_MDS_R_DID_PREPOPULATE, &req->r_req_flags)) {
      ...
      } else if (ceph_frag_is_leftmost(frag) && dfi->next_offset == 2) {
    ...
    if (rinfo->dir_nr > 0) {
      ...
      unsigned next_offset =
          req->r_reply_info.dir_end ? 2 : (fpos_off(rde->offset) + 1);
      err = note_last_dentry(fsc, dfi, rde->name, rde->name_len, next_offset);
      if (err) {
        ceph_mdsc_put_request(dfi->last_readdir);
        dfi->last_readdir = NULL;
        return err;
  ...
  doutc(cl, "%p %llx.%llx frag %x num %d pos %llx chunk first %llx\n", inode,
        ceph_vinop(inode), dfi->frag, rinfo->dir_nr, ctx->pos,
  ...
  for (; i < rinfo->dir_nr; i++) {
    ...
    if (rde->offset < ctx->pos) {
      pr_warn_client(cl, "%p %llx.%llx rde->offset 0x%llx ctx->pos 0x%llx\n",
    ...
    doutc(cl, "%p %llx.%llx (%d/%d) -> %llx '%.*s' %p\n", inode,
          ceph_vinop(inode), i, rinfo->dir_nr, ctx->pos, rde->name_len,
          rde->name, &rde->inode.in);

    if (!dir_emit(
            ctx, rde->name, rde->name_len,
  ...
  if (!ceph_frag_is_rightmost(dfi->frag)) {
    ...
    if (is_hash_order(ctx->pos)) {
      loff_t new_pos =
          ceph_make_fpos(ceph_frag_value(frag), dfi->next_offset, true);
      if (new_pos > ctx->pos)
        ctx->pos = new_pos;
      /* keep last_name */
    } else {
    ...
    if (!req->r_reply_info.head->is_dentry) {
      ...
      if (d_really_is_positive(dentry)) {
        ...
        if (d_unhashed(dentry)) {
          ...
          if (!parent || ceph_snap(parent) == CEPH_NOSNAP)
            d_add(dentry, NULL);
        }
      }
    }
 ...
 */
static struct dentry* ceph_lookup(struct inode* dir,
                                  struct dentry* dentry,
                                  unsigned int flags) {
  ...
  if (d_really_is_negative(dentry)) {
    ...
    doutc(cl, " dir %llx.%llx flags are 0x%lx\n", ceph_vinop(dir),
          ci->i_ceph_flags);
    if (strncmp(dentry->d_name.name, fsc->mount_options->snapdir_name,
                dentry->d_name.len) &&
        !is_root_ceph_dentry(dir, dentry) && ceph_test_mount_opt(fsc, DCACHE) &&
        ceph_snap(dir) == CEPH_NOSNAP && __ceph_dir_is_complete(ci) &&
        __ceph_caps_issued_mask_metric(ci, CEPH_CAP_FILE_SHARED, 1)) {
...
static int ceph_mknod(struct mnt_idmap* idmap,
                      struct inode* dir,
                      struct dentry* dentry,
                      umode_t mode,
                      dev_t rdev) {
  ...
  doutc(cl, "%p %llx.%llx/'%pd' dentry %p mode 0%ho rdev %d\n", dir,
        ceph_vinop(dir), dentry, dentry, mode, rdev);
  ...
  req->r_dentry_drop =
      CEPH_CAP_FILE_SHARED | CEPH_CAP_AUTH_EXCL | CEPH_CAP_XATTR_EXCL;
...
static int ceph_create(struct mnt_idmap* idmap,
                       struct inode* dir,
                       struct dentry* dentry,
                       umode_t mode) {
...
static int prep_encrypted_symlink_target(struct ceph_mds_request* req,
  ...
  len = base64_encode(osd_link.name, osd_link.len, req->r_path2, false,
                      BASE64_IMAP);
  req->r_path2[len] = '\0';
out:
...
static int ceph_symlink(struct mnt_idmap* idmap,
                        struct inode* dir,
                        struct dentry* dentry,
                        const char* dest) {
  struct ceph_mds_client* mdsc = ceph_sb_to_mdsc(dir->i_sb);
...
static struct dentry* ceph_mkdir(struct mnt_idmap* idmap,
                                 struct inode* dir,
                                 struct dentry* dentry,
                                 umode_t mode) {
  struct ceph_mds_client* mdsc = ceph_sb_to_mdsc(dir->i_sb);
  ...
  err = ceph_mdsc_do_request(mdsc, dir, req);
  if (!err && !req->r_reply_info.head->is_target &&
...
static int ceph_link(struct dentry* old_dentry,
                     struct inode* dir,
                     struct dentry* dentry) {
  struct ceph_mds_client* mdsc = ceph_sb_to_mdsc(dir->i_sb);
  struct ceph_client* cl = mdsc->fsc->client;
...
static void ceph_async_unlink_cb(struct ceph_mds_client* mdsc,
  ...
  struct ceph_dentry_info* di = ceph_dentry(dentry);
  int result =
      req->r_err ? req->r_err : le32_to_cpu(req->r_reply_info.head->result);
...
static int ceph_unlink(struct inode* dir, struct dentry* dentry) {
  ...
  if (ceph_snap(dir) == CEPH_SNAPDIR) {
    ...
    op = d_is_dir(dentry) ? CEPH_MDS_OP_RMDIR : CEPH_MDS_OP_UNLINK;
  } else
    goto out;
...
retry:
  ...
  if (try_async && op == CEPH_MDS_OP_UNLINK &&
    ...
    doutc(cl, "async unlink on %llx.%llx/'%pd' caps=%s", ceph_vinop(dir),
          dentry, ceph_cap_string(req->r_dir_caps));
    ...
    spin_lock(&fsc->async_unlink_conflict_lock);
    hash_add_rcu(fsc->async_unlink_conflict, &di->hnode, dentry->d_name.hash);
    spin_unlock(&fsc->async_unlink_conflict_lock);
...
static int ceph_rename(struct mnt_idmap* idmap,
  ...
  if ((old_dir != new_dir) && (!ceph_quota_is_same_realm(old_dir, new_dir)))
    return -EXDEV;
  ...
  doutc(cl, "%llx.%llx/'%pd' to %llx.%llx/'%pd'\n", ceph_vinop(old_dir),
        old_dentry, ceph_vinop(new_dir), new_dentry);
  req = ceph_mdsc_create_request(mdsc, op, USE_AUTH_MDS);
  if (IS_ERR(req))
    return PTR_ERR(req);
  ihold(old_dir);
  ...
  if (d_really_is_positive(new_dentry)) {
    req->r_inode_drop = ceph_drop_caps_for_unlink(d_inode(new_dentry));
  }
...
static unsigned long __dentry_leases_walk(struct ceph_mds_client* mdsc,
  ...
  while (!list_empty(&dispose)) {
    di = list_first_entry(&dispose, struct ceph_dentry_info, lease_list);
    dentry = di->dentry;
    spin_lock(&dentry->d_lock);
    ...
    if (di->flags & CEPH_DENTRY_REFERENCED) {
      spin_lock(&mdsc->dentry_list_lock);
      if (di->flags & CEPH_DENTRY_LEASE_LIST) {
        list_add_tail(&di->lease_list, &mdsc->dentry_leases);
      } else {
        __dentry_dir_lease_touch(mdsc, di);
      }
      spin_unlock(&mdsc->dentry_list_lock);
...
static int __dir_lease_check(const struct dentry* dentry,
  ...
  if (ret > 0) {
     ...
     * round robin manner */
    if (dentry->d_lockref.count > 0 || (di->flags & CEPH_DENTRY_REFERENCED))
      return TOUCH;
...
int ceph_trim_dentries(struct ceph_mds_client* mdsc) {
  ...
  if (mdsc->caps_use_max > 0 && mdsc->caps_use_count > mdsc->caps_use_max)
...
static bool __dentry_lease_is_valid(struct ceph_dentry_info* di) {
  ...
  if (session) {
    ...
    if (di->lease_gen == gen && time_before(jiffies, ttl) &&
...
static int dentry_lease_is_valid(struct dentry* dentry, unsigned int flags) {
  ...
  if (di && __dentry_lease_is_valid(di)) {
    ...
    if (di->lease_renew_after && time_after(jiffies, di->lease_renew_after)) {
      /*
  ...
  if (session) {
    ceph_mdsc_lease_send_msg(session, dentry, CEPH_MDS_LEASE_RENEW, seq);
 ...
 */
static int dir_lease_is_valid(struct inode* dir,
                              struct dentry* dentry,
                              struct ceph_mds_client* mdsc) {
  struct ceph_inode_info* ci = ceph_inode(dir);
  struct ceph_client* cl = mdsc->fsc->client;
  ...
  if (valid) {
    ...
    spin_lock(&dentry->d_lock);
    di = ceph_dentry(dentry);
    if (dir == d_inode(dentry->d_parent) && di &&
        di->lease_shared_gen == shared_gen)
      __ceph_dentry_dir_lease_touch(di);
    else
      valid = 0;
 ...
 */
static int ceph_d_revalidate(struct inode* dir,
  ...
  doutc(cl, "%p '%pd' inode %p offset 0x%llx nokey %d\n", dentry, dentry, inode,
        ceph_dentry(dentry)->offset, !!(dentry->d_flags & DCACHE_NOKEY_NAME));
 ...
 * with '-o dirstat'.
 */
static ssize_t ceph_read_dir(struct file* file,
                             char __user* buf,
                             size_t size,
                             loff_t* ppos) {
  struct ceph_dir_file_info* dfi = file->private_data;
  ...
  if (!dfi->dir_info) {
    ...
    dfi->dir_info_len =
        snprintf(dfi->dir_info, bufsize,
                 ...
                 ci->i_rfiles + ci->i_rsubdirs, ci->i_rfiles, ci->i_rsubdirs,
                 ci->i_rbytes, &ci->i_rctime);
  }
...
unsigned ceph_dentry_hash(struct inode* dir, struct dentry* dn) {
  ...
  switch (dci->i_dir_layout.dl_dir_hash) {
    ...
    default:
      spin_lock(&dn->d_lock);
      hash = ceph_str_hash(dci->i_dir_layout.dl_dir_hash, dn->d_name.name,
                           dn->d_name.len);
