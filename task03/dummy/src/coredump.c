...
static __printf(2, 0) int cn_vprintf(struct core_name* cn,
                                     const char* fmt,
                                     va_list arg) {
  int free, need;
  va_list arg_copy;
...
static __printf(2,
                3) int cn_printf(struct core_name* cn, const char* fmt, ...) {
  va_list arg;
  int ret;
...
static __printf(2, 3) int cn_esc_printf(struct core_name* cn,
                                        const char* fmt,
                                        ...) {
  int cur = cn->used;
  va_list arg;
 ...
 */
static bool coredump_parse(struct core_name* cn,
  ...
  switch (cn->core_type) {
    ...
    case COREDUMP_SOCK: {
      ...
      if (strchr(cn->corename, ' ')) {
        coredump_report_failure("Coredump socket may not %s contain spaces",
                                cn->corename);
        return false;
      }
      ...
      if (name_contains_dotdot(cn->corename)) {
        coredump_report_failure(
            "Coredump socket may not %s contain '..' spaces", cn->corename);
        return false;
      }

      if (strlen(cn->corename) >= UNIX_PATH_MAX) {
        coredump_report_failure("Coredump socket path %s too long",
                                cn->corename);
        return false;
      }
  ...
  while (*pat_ptr) {
    ...
    if (*pat_ptr != '%') {
      ...
      switch (*++pat_ptr) {
        ...
        /* global pid */
        case 'P':
          err = cn_printf(cn, "%d", task_tgid_nr(current));
          break;
        case 'i':
        ...
        case 'h':
          down_read(&uts_sem);
          err = cn_esc_printf(cn, "%s", utsname()->nodename);
          up_read(&uts_sem);
          break;
...
static bool coredump_sock_connect(struct core_name* cn,
  ...
  if (retval) {
    if (retval == -EAGAIN)
      coredump_report_failure("Coredump socket %s receive queue full",
                              addr.sun_path);
    else
      coredump_report_failure("Coredump socket connection %s failed %d",
...
static bool coredump_socket(struct core_name* cn,
                            struct coredump_params* cprm) {
...
static inline bool coredump_force_suid_safe(
    const struct coredump_params* cprm) {
  /* Require nonrelative corefile path and be extra careful. */
  return cprm->dumpable == TASK_DUMPABLE_ROOT;
}

static bool coredump_file(struct core_name* cn,
                          struct coredump_params* cprm,
  ...
  if (coredump_force_suid_safe(cprm)) {
     ...
     */
    scoped_with_init_fs() file = filp_open(cn->corename, open_flags, 0600);
  } else {
  ...
  if (!vfsuid_eq_kuid(i_uid_into_vfsuid(idmap, inode), current_fsuid())) {
    coredump_report_failure(
        "Core dump to %s aborted: cannot preserve file owner", cn->corename);
    return false;
  }
...
static bool coredump_pipe(struct core_name* cn,
                          struct coredump_params* cprm,
                          size_t* argv,
                          int argc) {
  int argi;
  ...
  sub_info =
      call_usermodehelper_setup(helper_argv[0], helper_argv, NULL, GFP_KERNEL,
...
static void coredump_cleanup(struct core_name* cn,
                             struct coredump_params* cprm) {
  if (cprm->file)
...
static void do_coredump(struct core_name* cn,
                        struct coredump_params* cprm,
                        size_t** argv,
                        int* argc,
                        const struct linux_binfmt* binfmt) {
...
int dump_user_range(struct coredump_params* cprm,
                    unsigned long start,
                    unsigned long len) {
  unsigned long addr;
...
static int proc_dostring_coredump(const struct ctl_table* table,
                                  int write,
                                  void* buffer,
                                  size_t* lenp,
                                  loff_t* ppos) {
...
#define FILTER(type) (mm_flags & (1UL << MMF_DUMP_##type))
  ...
  if (vma->vm_flags & VM_SHARED) {
    if (file_inode(vma->vm_file)->i_nlink == 0 ? FILTER(ANON_SHARED)
 ...
 */
static struct vm_area_struct* coredump_next_vma(
...
static bool dump_vma_snapshot(struct coredump_params* cprm) {
  ...
  if (core_sort_vma)
    sort(cprm->vma_meta, cprm->vma_count, sizeof(*cprm->vma_meta), cmp_vma_size,
         NULL);
