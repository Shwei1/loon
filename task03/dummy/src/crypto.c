...
static int ceph_crypt_set_context(struct inode* inode,
                                  const void* ctx,
                                  size_t len,
                                  void* fs_data) {
  int ret;
...
static const union fscrypt_policy* ceph_get_dummy_policy(
...
int ceph_fscrypt_prepare_context(struct inode* dir,
                                 struct inode* inode,
 ...
 */
int ceph_fname_to_usr(const struct ceph_fname* fname,
                      unsigned char* tname,
  ...
  /* Handle the special case of snapshot names that start with '_' */
  if ((ceph_snap(dir) == CEPH_SNAPDIR) && (name_len > 0) && (name[0] == '_')) {
  ...
  if (dir != fname->dir) {
    ...
    name_len = snprintf(tmp_buf, sizeof(tmp_buf), "_%.*s_%llu", oname->len,
                        oname->name, dir->i_ino);
 ...
 */
int ceph_fscrypt_decrypt_pages(struct inode* inode,
                               struct page** page,
                               u64 off,
                               int len) {
  int i, num_blocks;
  ...
  for (i = 0; i < num_blocks; ++i) {
    ...
    fret = ceph_fscrypt_decrypt_block_inplace(
        inode, page[pgidx], CEPH_FSCRYPT_BLOCK_SIZE, pgoffs, baseblk + i);
    if (fret < 0) {
      if (ret == 0)
        ret = fret;
      break;
 ...
 * block.
 */
int ceph_fscrypt_decrypt_extents(struct inode* inode,
                                 struct page** page,
                                 u64 off,
  ...
  ceph_calc_file_object_mapping(&ci->i_layout, off, map[0].len, &objno, &objoff,
                                &xlen);
  ...
  for (i = 0; i < ext_cnt; ++i) {
    ...
    if ((ext->off | ext->len) & ~CEPH_FSCRYPT_BLOCK_MASK) {
      pr_warn_client(cl,
                     ...
                     inode, ceph_vinop(inode), i, ext->off, ext->len);
      return -EIO;
    }
    fret =
        ceph_fscrypt_decrypt_pages(inode, &page[pgidx], off + pgsoff, ext->len);
 ...
 */
int ceph_fscrypt_encrypt_pages(struct inode* inode,
                               struct page** page,
                               u64 off,
                               int len) {
  int i, num_blocks;
